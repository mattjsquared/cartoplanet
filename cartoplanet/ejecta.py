...
def Xie_figure10c(grid=False) -> tuple:
  ...
  elevation = ds_profile['elevation'].values
  ### //Plot//
  depth = -elevation + ds_profile['thickness_total'].item()
  with plt.rc_context({
      'font.family': 'Myriad Pro',
      'figure.dpi': 300,
      'lines.linewidth': 1.5,
...
def compute_SOI_params(
  ...
  -------
  out : tuple
    Tuple containing pre-computed per-SOI parameters for ejecta thickness calculations. arameters are:
      ...
      - ejecta_model                : EjectaModel
  """
  ##TODO: Figure out how to handle overlapping or non-contiguous SOIs -- are there mass double-counting or gaps in coverage?
  ### //Validate inputs//
  if (nSOI is not None) and (verbose > 0):
  ...
  max_dist   = dist_limit if (radius_cutoff is None) else min([radius_cutoff*R_km, dist_limit]) #[km]
  ...
  dist_km = np.linspace(min_dist, max_dist, nSOI) if (rSOI is None) else np.asarray(rSOI) #[km]
  if rSOI is None:
    dist_km = np.linspace(min_dist, max_dist, nSOI)  #[km]
  else:
    dist_km = np.asarray(rSOI) #[km]
  # Handle out-of-bounds distances
  in_bounds    = (dist_km >= min_dist) & (dist_km <= max_dist)
  if any(~in_bounds) and (verbose > 0):
    warnings.warn(f"{ds_basin.basin.item()} (R={R_km:.2f}km, Rat={Rat_km:.2f}km): All `dist_km` must be {min_dist:.2f} ≤ `dist_km` ≤ {max_dist:.2f}. Setting {(~in_bounds).sum():d} out-of-bounds points to NaN.")
  dist_km_full = dist_km.copy()     #preserve full array to return later
  dist_km      = dist_km[in_bounds] #cull invalid points for calculations
  ...
  lsc_distance_m = (a_rLSC * Rat_km**b_rLSC) * 1e3  #r_LSC [m] -- Eq. 10 of Xie et al. (2020) [**this is only valid for complex and larger craters**] ##TODO: check this re: simple/complex craters
  ...
  # mass_lower         = 1e-18 * mass_upper                                                         #m_l [g] ##TODO: Why did I comment this out?
  ...
  ### //Return//
  def _restore(arr, out_of_bounds=np.nan):
    """Helper function to fill out-of-bounds points with NaN for returned arrays."""
    restored = np.full_like(in_bounds, out_of_bounds, dtype=np.float64)
    restored[in_bounds] = arr
    return restored
  out = (
    # Scalars
    pre_frag_radius,
    exp_sec_transient_radius,
    # Arrays -- shape (nSOI,)
    dist_km_full,
    _restore(mass_lower),
    _restore(mass_upper),
    _restore(mass_norm_constant),
    _restore(soi_area),
    _restore(thickness_primary),
    _restore(pre_sec_transient_radius1),
    _restore(pre_sec_transient_radius2),
    _restore(pre_central_effective_depth),
    # Objects
    ejecta_model
  )
  return out
...
def precompute_SOI(
  ...
  cache : dict
    Dictionary containing pre-computed per-SOI parameters for ejecta thickness calculations. Cached parameters are:
      ...
      - ejecta_model : EjectaModel
  """
  pre_frag_radius, exp_sec_transient_radius, dist_km, mass_lower, mass_upper, mass_norm_constant, soi_area, thickness_primary, pre_sec_transient_radius1, pre_sec_transient_radius2, pre_central_effective_depth, ejecta_model = compute_SOI_params(
    ds_basin=ds_basin,
    nSOI=nSOI,
    rSOI=rSOI,
...
def precompute_SOI_xarray(
  ...
  """
  Compute per-basin and -SOI parameters needed for ballistic sedimentation calculations.
  ...
  grid_lats, grid_lons : list or np.ndarray
    Latitudes and longitudes of the grid points at which to compute the SOI parameters.
  radius_cutoff : float or None
    If set, maximum distance = radius_cutoff * R_km.
  ejecta_model : dict or EjectaModel
    If dict, must contain the following keys (with values as described in the EjectaModel class):
  ...
  -------
  ds_cache : xr.Dataset
    xarray Dataset containing pre-computed per-basin, per-SOI ballistic sedimentation parameters with variables:
      - R                           : (nbasin,)
      - Rat                         : (nbasin,)
      - clat                        : (nbasin,)
      - clon                        : (nbasin,)
      - order                       : (nbasin,)
      - pre_frag_radius             : (nbasin,)
      - exp_sec_transient_radius    : (nbasin,)
      - dist_km                     : (nlat, nlon, nbasin)
      - mass_lower                  : (nlat, nlon, nbasin)
      - mass_upper                  : (nlat, nlon, nbasin)
      - mass_norm_constant          : (nlat, nlon, nbasin)
      - soi_area                    : (nlat, nlon, nbasin)
      - thickness_primary           : (nlat, nlon, nbasin)
      - pre_sec_transient_radius1   : (nlat, nlon, nbasin)
      - pre_sec_transient_radius2   : (nlat, nlon, nbasin)
      - pre_central_effective_depth : (nlat, nlon, nbasin)
    with coordinates:
      - lat   : grid_lat
      - lon   : grid_lon
      - basin : ds_basin['basin']
    and attributes:
      - ejecta_model : EjectaModel
  """
  ### //Set up computation//
  ...
  nlon   = len(grid_lons)
  nSOI   = nlat * nlon
  SOIlon, SOIlat = np.meshgrid(grid_lons, grid_lats)
  SOIlon = SOIlon.ravel(order='C')
  SOIlat = SOIlat.ravel(order='C')
  nbasin = len(ds_basin['basin'].values)
  # Initialize arrays
  pre_frag_radius             = np.zeros(nbasin)
  exp_sec_transient_radius    = np.zeros(nbasin)
  dist_km                     = np.zeros((nlat, nlon, nbasin))
  mass_lower                  = np.zeros((nlat, nlon, nbasin))
  mass_upper                  = np.zeros((nlat, nlon, nbasin))
  mass_norm_constant          = np.zeros((nlat, nlon, nbasin))
  soi_area                    = np.zeros((nlat, nlon, nbasin))
  thickness_primary           = np.zeros((nlat, nlon, nbasin))
  pre_sec_transient_radius1   = np.zeros((nlat, nlon, nbasin))
  pre_sec_transient_radius2   = np.zeros((nlat, nlon, nbasin))
  pre_central_effective_depth = np.zeros((nlat, nlon, nbasin))
  # Define helper for reshaping arrays
  _reshape = lambda arr: np.reshape(arr, (nlat, nlon), order='C')
  # Iterate through basins to compute ballistic sedimentation parameters
  for i, b in enumerate(ds_basin['basin'].values):
    basin = ds_basin.sel(basin=b)
    rSOI = great_circle_distance(SOIlat, SOIlon, basin.clat.item(), basin.clon.item())
    cache_b = precompute_SOI(
      ds_basin            = basin,
      rSOI                = rSOI,
      radius_cutoff       = radius_cutoff,
      ejecta_model        = ejecta_model,
    ...
    pre_frag_radius[i]                   = cache_b['pre_frag_radius']
    exp_sec_transient_radius[i]          = cache_b['exp_sec_transient_radius']
    dist_km[:, :, i]                     = _reshape(cache_b['dist_km'])
    mass_lower[:, :, i]                  = _reshape(cache_b['mass_lower'])
    mass_upper[:, :, i]                  = _reshape(cache_b['mass_upper'])
    mass_norm_constant[:, :, i]          = _reshape(cache_b['mass_norm_constant'])
    soi_area[:, :, i]                    = _reshape(cache_b['soi_area'])
    thickness_primary[:, :, i]           = _reshape(cache_b['thickness_primary'])
    pre_sec_transient_radius1[:, :, i]   = _reshape(cache_b['pre_sec_transient_radius1'])
    pre_sec_transient_radius2[:, :, i]   = _reshape(cache_b['pre_sec_transient_radius2'])
    pre_central_effective_depth[:, :, i] = _reshape(cache_b['pre_central_effective_depth'])
  ### //Build and return the xarray Dataset//
  ds_cache = xr.Dataset(
    {
      # SOI-independent scalars
      'R'                          : (('basin',), ds_basin['R'].values),
      'Rat'                        : (('basin',), ds_basin['Rat'].values),
      'clat'                       : (('basin',), ds_basin['clat'].values),
      'clon'                       : (('basin',), ds_basin['clon'].values),
      'order'                      : (('basin',), ds_basin['order'].values),
      'pre_frag_radius'            : (('basin',), pre_frag_radius),
      'exp_sec_transient_radius'   : (('basin',), exp_sec_transient_radius),
      # SOI-dependent arrays
      'dist_km'                    : (('lat', 'lon', 'basin'), dist_km),
      'mass_lower'                 : (('lat', 'lon', 'basin'), mass_lower),
      'mass_upper'                 : (('lat', 'lon', 'basin'), mass_upper),
      'mass_norm_constant'         : (('lat', 'lon', 'basin'), mass_norm_constant),
      'soi_area'                   : (('lat', 'lon', 'basin'), soi_area),
      'thickness_primary'          : (('lat', 'lon', 'basin'), thickness_primary),
      'pre_sec_transient_radius1'  : (('lat', 'lon', 'basin'), pre_sec_transient_radius1),
      'pre_sec_transient_radius2'  : (('lat', 'lon', 'basin'), pre_sec_transient_radius2),
      'pre_central_effective_depth': (('lat', 'lon', 'basin'), pre_central_effective_depth),
    },
    coords = {
      'lat'  : grid_lats,
      'lon'  : grid_lons,
      'basin': ds_basin['basin'].values
    },
    attrs = {
      # SOI- and basin-independent objects
      'ejecta_model': ejecta_model,
    }
  )
  return ds_cache
...
def compute_central_effective_depth(                                 #d_eff (Eq. 17 of Xie et al. (2020))
  ...
  cache : dict | xr.Dataset
    Dictionary or xarray Dataset containing pre-computed per-SOI parameters for ballistic sedimentation calculations, as returned by `precompute_SOI()` or `precompute_SOI_xarray()`.
  i_soi : int
  ...
  if isinstance(cache, dict):
    ...
    pre_sec1                 = np.atleast_1d(cache['pre_sec_transient_radius1'])[i_soi]
    pre_sec2                 = np.atleast_1d(cache['pre_sec_transient_radius2'])[i_soi]
    pre_deff                 = np.atleast_1d(cache['pre_central_effective_depth'])[i_soi]
  else: ##TODO: Need to refactor everything to handle caches more smoothly -- right now the xr compatibility below assumes that the cache is length-1
    pre_frag_radius          = cache['pre_frag_radius'].item()
    exp_sec_transient_radius = cache['exp_sec_transient_radius'].item()
    pre_sec1                 = cache['pre_sec_transient_radius1'].item()
    pre_sec2                 = cache['pre_sec_transient_radius2'].item()
    pre_deff                 = cache['pre_central_effective_depth'].item()
...
def compute_coverage_kernel(
  ...
  """
  Fast vectorized approximation of mass-continuous forms of Eq. 19 or 20 of Xie et al. (2020). Can be used to build a mass-space coverage kernel (if `depth_in` and `coverage_in` are both None) or to compute (1) coverage fraction(s) at `depth_in` or (2) depth(s) at `coverage_in`.
  ...
  soi_cache : dict | xr.Dataset
    Dictionary or xarray Dataset containing pre-computed per-SOI parameters for ejecta thickness calculations, as returned by `precompute_SOI()` or `precompute_SOI_xarray()`.
  i_soi : int
  ...
  depth_in : ndarray, optional
    If specified, `coverage_in` must be None and `coverage_fraction` will be computed exactly at the input depths. If None (default), `depth` will be computed as a mass-space grid of effective excavation depths, and `coverage_fraction` will be computed at those depths.
  coverage_in : ndarray, optional
    If specified, `depth_in` must be None and `depths` will be interpolated from a mass-space grid of effective excavation depths. If None (default), `coverage_fraction` will be computed on `depth`.
  
  Returns
  -------
  depth_out : ndarray
    Array of grid point depths. If `depth_in` is None, this is a mass-space grid of effective excavation depths. Otherwise, returns `depth_in`.
  ...
  if isinstance(soi_cache, dict):
    pre_frag_radius = soi_cache['pre_frag_radius']
    exp_sec_transient_radius = soi_cache['exp_sec_transient_radius']
    ml       = np.atleast_1d(soi_cache['mass_lower'])[i_soi]
    mh       = np.atleast_1d(soi_cache['mass_upper'])[i_soi]
    C        = np.atleast_1d(soi_cache['mass_norm_constant'])[i_soi]
    S        = np.atleast_1d(soi_cache['soi_area'])[i_soi]
    pthick   = np.atleast_1d(soi_cache['thickness_primary'])[i_soi]
    pre_sec1 = np.atleast_1d(soi_cache['pre_sec_transient_radius1'])[i_soi]
    pre_sec2 = np.atleast_1d(soi_cache['pre_sec_transient_radius2'])[i_soi]
    pre_deff = np.atleast_1d(soi_cache['pre_central_effective_depth'])[i_soi]
  else: ##TODO: Need to refactor everything to handle caches more smoothly -- right now the xr compatibility below assumes that the cache is length-1
    pre_frag_radius = soi_cache['pre_frag_radius'].item()
    exp_sec_transient_radius = soi_cache['exp_sec_transient_radius'].item()
    ml       = soi_cache['mass_lower'].item()
    mh       = soi_cache['mass_upper'].item()
    C        = soi_cache['mass_norm_constant'].item()
    S        = soi_cache['soi_area'].item()
    pthick   = soi_cache['thickness_primary'].item()
    pre_sec1 = soi_cache['pre_sec_transient_radius1'].item()
    pre_sec2 = soi_cache['pre_sec_transient_radius2'].item()
    pre_deff = soi_cache['pre_central_effective_depth'].item()
  ...
  try:
    ...
    b    = soi_cache.attrs['ejecta_model'].b
...
def build_mixing_kernel(
  ...
  kernel : dict or None
    ...
    -       : float   - total thickness of primary ejecta in this SOI
  ...
  if isinstance(cache, dict):
    pthick = np.atleast_1d(cache['thickness_primary'])[i_soi] #[m]
    mupper = np.atleast_1d(cache['mass_upper'])[i_soi]        #[g]
  else: ##TODO: Need to refactor everything to handle caches more smoothly -- right now the xr compatibility below assumes that the cache is length-1
    pthick = cache['thickness_primary'].item() #[m]
    mupper = cache['mass_upper'].item()        #[g]
...
def compute_ejecta_mixing(
  ...
  Tprimary               = kernel['thickness_primary']      #[m]
...
def compute_ejecta_mixing_multi_basin(
  ...
  ds_profile : xr.Dataset
    ...
    and variables:
    - thickness_total   : total thickness of primary ejecta from all basins at this location
    - thickness_primary : thickness of primary ejecta from each basin at this location
  ...
  for b in tqdm(ds_caches['basin'].values, desc="Pre-computing basin SOI caches", disable=verbose<1):
    ...
    if rSOI < basin['R'].item(): ## TODO: should this use R or Rat, or something else? I'm assuming all basins would reset stratigraphy but that wouldn't necessarily be true near the rim
      ...
      thicknesses_primary.extend(reset_thicknesses) #stratigraphy is reset, so all older basins have zero thickness
      break
    try:
    ...
    caches.append(cache)
    thicknesses_primary.append(cache['thickness_primary'][0])     #[m]
  ds_caches['soi_cache']         = (('basin',), np.array(caches, dtype=object))
  ds_caches['thickness_primary'] = (('basin',), np.array(thicknesses_primary)) #[m]
  Tprimary_total                 = ds_caches['thickness_primary'].sum()        #[m]
  ...
  ds_profile = xr.Dataset(
    {
      'thickness_total'  : (('lat', 'lon'), np.atleast_2d(Tprimary_total)), #[m]
      'thickness_primary': (('lat', 'lon', 'basin'), np.reshape(np.concatenate(([0], basins_ordered['thickness_primary'].values)), (1, 1, -1))), #[m]
    },
    coords = {
  ...
  thickness_primary_cumulative = ds_profile['thickness_primary'].cumsum(dim='basin')
  ...
  for b in tqdm(basins_ordered['basin'].values, desc="Computing each basin's vertical mixing", disable=verbose<1):
    ...
    basin             = ds_profile.sel(basin=b)
    abundances        = compute_ejecta_mixing(kernel, elevation, abundances, surface_elevation=thickness_primary_cumulative.sel(basin=b).item()-basin['thickness_primary'].item())
  ...
  ds_profile['abundance'] = ds_profile['abundance'].where(ds_profile['elevation'] <= ds_profile['thickness_total'], np.nan)
  return ds_profile


def compute_ejecta_mixing_multi_basin_precached(
    cache: dict,
    basins: list,
    profile_lat: float,
    profile_lon: float,
  ...
  ----------
  cache : dict
    Precomputed per-basin ballistic sedimentation parameters for the given SOI, with the same keys as the data_vars returned by `precompute_SOI_xarray()`.
    Must be sorted by 'order' in ascending order (oldest -> youngest).
  basins : list
    List of basin names corresponding to the arrays in `cache`. Must be in the same order as the 'basin' dimension of `cache`.
  profile_lat, profile_lon : float
    Coordinates of the point at which to compute the vertical mixing profile.
  ejecta_model : EjectaModel
    EjectaModel instance fetched from the output of `precompute_SOI_xarray()`, containing the parameters used for the ballistic sedimentation calculations.
  elevation : optional, ndarray or None
    Elevation grid in meters for the vertical mixing profile. If `None`, elevation grid will be handled automatically.
  skip_intermediate_abundance : bool
    If True, skip computation of `abundance_iflast` and exclude it from the output. Default is False.
  preimpact_label : str, optional
    Name for the mixing component corresponding to local materials that predate the first impact. If `None`, defaults to 'preimpact'.
  verbose : bool
    Whether to print progress bars for the pre-computation and mixing steps. Default is False.
  ...
  ds_profile : xr.Dataset
    ...
    - abundance_iflast  : area or volume fraction of each basin's primary ejecta at each elevation if there were no subsequent impacts
  """
  is_ascending = np.all(cache['order'][:-1] <= cache['order'][1:])
  if not is_ascending:
  ...
  for idx in tqdm(
    ...
    R_b     = cache['R'][idx]
    order_b = cache['order'][idx]
    if rSOI_b < R_b: ## TODO: should this use R or Rat, or something else? I'm assuming all basins would reset stratigraphy but that wouldn't necessarily be true near the rim
      # This point is inside the rim of this basin, so all previous ejecta is erased and none is deposited here by this basin; break
      cutoff_order = order_b
      break
  nonzero_basins               = (cache['order'] > cutoff_order) & np.isfinite(cache['thickness_primary'])
  cache['thickness_primary']   = np.where(nonzero_basins, cache['thickness_primary'], 0.0) #[m] -- set thickness to zero for basins that are stratigraphically reset or out of bounds
  Tprimary_total               = cache['thickness_primary'].sum() #[m]
  thickness_primary_cumulative = np.cumsum(cache['thickness_primary']) #[m]
  ### //Set up the arrays//
  # Set up the elevation grid
  if elevation is None:
    dz              = .5                                                          #[m]
  ...
  for idx in tqdm(
    idx_basins,
    desc        = "Computing each basin's vertical mixing",
    mininterval = 0.5, 
    disable     = verbose < 1
  ):
    cache_b      = {var: cache[var][idx] for var in cache.keys()}
    cache_b['ejecta_model'] = ejecta_model
    reset_b      = cache_b['order'] <= cutoff_order
    surface_elev = thickness_primary_cumulative[idx] - cache_b['thickness_primary']
    if reset_b:
      # This basin's ejecta is erased at this point by a later impact; add a zero column and skip the mixing computation
      abundances        = np.column_stack([abundances, np.zeros(len(elevation))])
      abundances_iflast = np.column_stack([abundances_iflast, np.full(len(elevation), np.nan)])
      continue
    kernel       = build_mixing_kernel(cache_b, 0)
    abundances   = compute_ejecta_mixing(kernel, elevation, abundances, surface_elevation=surface_elev)
    if not skip_intermediate_abundance:
      abundances_iflast_thisbasin                                   = abundances[:, -1].copy()[:, None]
      abundances_iflast_thisbasin[abundances_iflast_thisbasin == 0] = np.nan
      abundances_iflast                                             = np.concatenate((abundances_iflast, abundances_iflast_thisbasin), axis=1)
  above_surface = elevation > Tprimary_total
  abundances    = np.where(above_surface[:, None], np.nan, abundances) 
  # Define the output dataset
  pre_lbl    = preimpact_label or 'preimpact'
  ds_profile = xr.Dataset(
    {
      'thickness_total'  : (('lat', 'lon'), np.atleast_2d(Tprimary_total)), #[m]
      'thickness_primary': (('lat', 'lon', 'basin'), np.reshape(np.concatenate(([0], cache['thickness_primary'])), (1, 1, -1))), #[m]
      'abundance'        : (('lat', 'lon', 'basin', 'elevation'), np.reshape(abundances.T, (1, 1, abundances.shape[1], abundances.shape[0])))
    },
    coords = {
      'lat'      : np.asarray([profile_lat]),
      'lon'      : np.asarray([profile_lon]),
      'basin'    : np.concatenate(([pre_lbl], basins)),
      'elevation': elevation, #[m]
    },
  )
  if not skip_intermediate_abundance:
  ...
  ds_profiles = xr.Dataset(
    {
      ...
      'thickness_primary': (('lat', 'lon', 'basin'), thickness_primary),
      'abundance':         (('lat', 'lon', 'basin', 'elevation'), np.flip(abundance, axis=-1)),
    },
    coords = {
      'lat': grid_lats,
      'lon': grid_lons,
